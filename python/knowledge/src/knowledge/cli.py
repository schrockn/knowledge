...
@click.argument("text")
def add_todo(text):
    """Add a todo item to today's daily todo list."""
    repo_root = find_repo_root()
    todo_file = repo_root / "DAILY_TODO.md"

    current_date = datetime.now().strftime("%Y-%m-%d")
    today_header = f"## {current_date}"

    # Create file if it doesn't exist
    if not todo_file.exists():
        content = f"# Daily TODO\n\n{today_header}\n\n- [ ] {text}\n"
        todo_file.write_text(content)
        click.echo(f"Created DAILY_TODO.md and added: {text}")
        return
    ...
    if today_index == -1:
        # Add new section for today at the top (after title)
        title_index = 0
        for i, line in enumerate(lines):
        ...
        lines.insert(insert_index + 1, today_header)
        lines.insert(insert_index + 2, "")
        lines.insert(insert_index + 3, f"- [ ] {text}")
    else:
        # Find where to insert the todo under today's section
        insert_index = today_index + 1
...
def ls_shower_thought():
    ...
    thoughts = sorted(
        shower_thoughts_dir.glob("*.md"), key=lambda x: x.stat().st_mtime, reverse=True
    )
    if not thoughts:
...
def ls_task():
    ...
    personal_tasks = (
        sorted((tasks_dir / "personal").glob("*.md")) if (tasks_dir / "personal").exists() else []
    )
...
def check_md(fix):
    ...
    try:
        ...
        if result.returncode != 0:
            if not fix:
                click.echo(
                    "Some files need formatting. Run 'knowledge check md --fix' to fix them."
                )
            else:
